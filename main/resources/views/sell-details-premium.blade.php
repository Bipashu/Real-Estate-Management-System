...
@if($iscomments)
<h6>Comments</h6>
<div class="p-5 shadow rounded mb-3 p-5">
...
@endif
<form action="{{route('sellcomment')}}" method="POST" class="rounded bg-white shadow p-5" >
    @csrf
...
<form action="{{ route('Bid')}}" method="POST" class="rounded bg-white shadow p-5" >
    @csrf
    @if($isbids)
     <h6>You have already bidded for this property. <a href={{"/viewbid/".$property->id}} class="text-decoration-none">View your bid here.</a></h6>
     @else
    <input type="hidden" name="id" value="{{$property->id}}">
<input type="text" class="form-control bg-light border border-white w-50" id="floatingInput" name="amount" placeholder="bid amount...">
<button type="submit"class="btn submit-btn w-25 my-4 text-white" style="background-color:#148c3a;">PLace Bid</button>
     @endif             
</form>
</div>
</div>              
