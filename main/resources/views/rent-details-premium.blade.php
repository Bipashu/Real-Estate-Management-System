...
@endif
<form action="{{route('rentcomment')}}" method="POST" class=" rounded bg-white shadow p-5" >
    @csrf
...
@if($isbids)
<p class="h6">You have already applied for this property.
</p>
@else
                        <form action="{{ route('applyForRent') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{$property->id}}">
                       <button type="submit" class="btn submit-btn rounded  shadow" style="background-color:#148c3a;">Apply for a rent</button>
                       </form>
@endif
                       </div>
</div>
